...
export const Web2Admin = () => {
  ...
  const handleResetElection = async () => {
    try {
      showToast('Resetting election...', 'warning');
      const result = await ApiService.resetElection();
      showToast(`Election reset successfully! ${result.details.studentsReset} students reset, ${result.details.delegateVotesDeleted + result.details.councilVotesDeleted} votes deleted.`, 'success');
      await loadData();
      setShowResetModal(false);
    } catch (error) {
      showToast(`Failed to reset election: ${error.message}`, 'error');
      setShowResetModal(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <ConfirmationModal
        ...
        message={`⚠️ WARNING: This will permanently delete ALL election data including:

• All votes (delegate and council)
• All candidate registrations
• All parties
• All elected delegates
